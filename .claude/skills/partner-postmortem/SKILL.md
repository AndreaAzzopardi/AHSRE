...
Markdown by default. If the user wants the distributable document (docx or Google Doc), follow the standard's Part 6 design rules, which mirror the INC-10938 executive Doc exactly: Inter Tight, all-black text with **no accent colour**, 17pt bold title, 12pt bold Title-Case section headers, 10pt body, en-dash bullets with bold labels, `#edf0f6` header/label cells with 1pt `#bfbfbf` borders, 8pt timeline cells, italic 8pt lead line and confidentiality footer, yellow `#ffff00` highlight only on pending `[HH:MM]` [COMMS] rows. For docx, run the generate → validate → PDF → image QA pipeline before delivering. For Google Docs via the Drive connector, use inline-styled HTML (`contentMimeType: text/html`); the connector is create-only, the font and the page footer must be applied manually in the Doc afterwards.
