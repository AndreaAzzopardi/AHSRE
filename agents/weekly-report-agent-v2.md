...
## Step 2I — Incident servicing & escalation split (DEDICATED ROUTINE — not part of the main nightly run)

> **⚠ Runs in its own cloud routine ("Servicing Cache — Nightly", 22:03 UTC), NOT in the main
...
**Leads lag behind reality — but re-fetching several weeks in one session is what killed the combined run. Rotate instead:** each night, in addition to the current week, recompute `total` + `leads` (procedure 1 below) for **exactly ONE of the 3 most recent complete weeks**, chosen by rotation: `index = day_of_year % 3` (0 = most recent complete week, 1 = second, 2 = third). Every complete week in the lag window is thus refreshed at least every 3 days — enough for the ~days-scale lead-reassignment lag — at a third of the token cost. **Escalation data of a finalised week stays frozen** — escalations are stamped by when the page fired and transitions are immutable, so those numbers cannot change retroactively.

**Token discipline (this is why the split exists):** fetch `incident_list` pages one at a time with `page_size: 50` and immediately reduce each page to `(reported_at, visibility, lead name)` tuples — never accumulate raw incident payloads. Same for escalation lists/shows: extract the verdict, discard the payload.

For the current week W (Monday 00:00 UTC → now):

1. **Lead histogram (recomputed wholesale, cheap ~5 calls)** — `incident_list` with `created_after=W`, `created_before=W+6d` (**⚠ `created_before` is INCLUSIVE of the entire end date — pass the SUNDAY, never W+7d, or the window swallows all of next Monday**; this inflated the w/c 6 Jul finalisation by +33 on 13 Jul), `page_size=50`, `include: ["roles"]`, paginate to `has_more=false`. Keep only incidents with `reported_at` strictly inside `[W, W+7d)` — apply this filter on EVERY pass, especially the finalisation pass after the week has closed — and `visibility == "public"`. Build `{lead name: count}` from the Incident Lead role; unassigned → `"NO LEAD"`. `total` = kept count. **Cross-check:** `total` ≈ Step 2C's `incident_volume.total` ±2; warn if it diverges by more, still write.
...
Feeds the **Response Quality** slide from `cache/response_quality_cache.json` (added 12 Jul 2026; **reworked same day** — the original detection-source metric was dropped as misleading and must NOT be re-fetched). Load the file and **preserve `method`** — never rewrite it. Needs the incident.io, Slack and Intercom connectors. Every run maintains the CURRENT week's row (`"partial": true` until the week ends and has been refreshed once after week end).

**⚠ incident.io `created_before` is inclusive of the entire end date — always pass the SUNDAY (W+6d), never W+7d, or the window swallows all of next Monday** (this bug inflated the first backfill by up to +39/week).

For the current week W (Monday):

1. **Cause of Incident (3 stats calls)** — for W **and the 2 most recent complete weeks** (cause tagging lags triage — the in-progress week's "None" runs ~45% and drains to <8% as weeks settle): `incident_stats` with `created_after=<monday>`, `created_before=<sunday>`, `group_by: ["custom_field:01KQC810AEHE7YEQ9MG04376QF"]`, `mode: ["standard","retrospective"]`, `status_category: ["triage","active","post-incident","closed","paused"]`, `max_incident_ids_per_group: 250`, `sort_groups_by: "count"`. Store per week: `cause = {"total": total_records, "causes": {"<label>": {"count": n, "refs": [sample_ids]}}}` — **keep the refs** (incl. the "None" group's) so any cause can be drilled into later. The field is multi-select: cause counts can sum above `total`; never "reconcile" that.

2. **Time to inform partners (2–3 Slack searches + incremental incident_show)** — Slack message search (never per-channel history paging) for `"FT Incident for internal reference" after:<W−7d>` plus the looser variant `"FT Incident" after:<W−7d>`. Keep only hits in channels whose name starts with **`ftcrm`** (golive-* and all other channels are NOT partner comms). Extract (INC ref, channel, author, ts); dedupe to the EARLIEST mention per incident — multi-brand incidents count ONCE. For refs not already in the cache, `incident_show` → `reported_at`, `severity`; `delay_min` = mention ts − reported_at (discard joins with delay < −60min or > 14 days into no entry + a note). Append new entries to the touched weeks' `inform.entries` (bucketed by reported_at week), keep existing ones, recompute `informed`/`median_min`/`p90_min` per touched week.

3. **Partner MTTR from Intercom (1 search + occasional get_conversation)** — `search_conversations` with `team_assignee_id: 50045975`, `created_at >= W−21d`; paginate. Per conversation: START = `statistics.first_assignment_at` (arrival at the SOC/SRE inbox) fallback `created_at` (record `start_basis`); CLOSE = `statistics.last_close_at`; P1 = tag `193658` present; if the `statistics` block is null, `get_conversation` and recover close from `conversation_parts` (part_type "close", LAST one) and first assignment from the earliest assignment part. Never use `sla_applied.sla_status`. Closed: P1 → raw hours START→CLOSE (24/7); non-P1 → office minutes Mon–Fri 09:00–17:00 UTC (compute in Python). Open: count only. Rebuild the affected weeks' rows wholesale (bucket by START week): `mttr = {"p1": {closed, open, median_h, p90_h}, "other": {closed, open, median_office_min, p90_office_min}, "items": [...]}`.

4. **P1 timestamp chain (1 call, kept for deep-dives — not on the slide)** — `incident_list` severity P1, window W..W+6d, `include: ["timestamps","durations"]`; keep `reported_at` ∈ [W, W+7d) and public only; per P1 minutes from "Reported at": `identify_min`, `fixed_min`, `resolve_min` (fallback duration÷60). Store as the week's `p1` list sorted by reported_at. ("Accepted at" is unstamped since ~5 Jun 2026 — do not use it.)

Save with `indent=1`. **Data integrity guard:** a failed fetch (any of the three sources) never deletes or zeroes an existing week or metric key — skip that metric, keep the rest, and report. Gate additions: file parses, no week key removed, current week `cause.total > 0`, and every stored duration/delay ≥ 0.
...
- [ ] Response quality (Step 2J): **NOT part of this run** — same dedicated routine maintains `cache/response_quality_cache.json` (Cause-of-Incident stats with refs, ftcrm Slack time-to-inform, Intercom partner MTTR from first inbox assignment, P1 timestamp chain); `method` preserved; failed fetches never zero a week or metric key

---

## Step 8B — MANDATORY completion gate (no partial runs)

**This step is NOT optional and the checklist above is NOT sufficient.** On 2026-06-25 a run refreshed some incident.io keys but silently skipped `engineer_workload`, `mtta` and all Intercom steps, then committed stale data. The checklist passed because the keys *existed* — it never checked they were *this run's* data. These two gates exist to make that impossible. **Do not proceed to commit if either gate fails.**
...
### Gate 2 — Deterministic verifier (script, blocks the commit)
After Step 6 (cache write) and Gate 1, run:
```python
import subprocess, os
repo = subprocess.run(["git","rev-parse","--show-toplevel"], capture_output=True, text=True).stdout.strip()
r = subprocess.run(["python3", os.path.join(repo,"agents","verify_current_week.py")], capture_output=True, text=True)
print(r.stdout); print(r.stderr)
# r.returncode == 0 → safe to commit;  != 0 → a partial/stale run was detected.
```
`agents/verify_current_week.py` cross-checks the current week against itself (no MCP): completeness of all expected keys, `engineer_workload` count vs `alert_volume.Intercom` (within tolerance), sanity bounds, and freshness. **If it exits non-zero, DO NOT commit.** Identify the stale key from its output, re-run that fetch step, regenerate, and re-run the verifier until it exits 0. Treat its WARNINGS as required follow-ups (e.g. a `partner_tickets` warning means the Intercom steps need re-running), not noise.

**Only when Gate 2 exits 0 may you generate the HTML (Step 9) and commit.** The remote trigger chains the commit on this verifier's exit code, so a failing run will refuse to commit regardless.
